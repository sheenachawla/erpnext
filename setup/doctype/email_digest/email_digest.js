...
cur_frm.cscript.refresh = function(doc, dt, dn) {
	...
	cur_frm.add_custom_button('View Now', function() {
		doc = locals[dt][dn];
		if(doc.__unsaved != 1) {
			$c_obj(wn.model.get_doclist(dt, dn), 'get_digest_msg', '', function(r, rt) {
				if(r.exc) {
					msgprint(err_msg);
					console.log(r.exc);
				} else {
	...
	cur_frm.add_custom_button('Send Now', function() {
		doc = locals[dt][dn];
		if(doc.__unsaved != 1) {
			$c_obj(wn.model.get_doclist(dt, dn), 'send', '', function(r, rt) {
				if(r.exc) {
					msgprint(err_msg);
					console.log(r.exc);
				} else {
...
cur_frm.cscript.addremove_recipients = function(doc, dt, dn) {
	// Get profile list
	$c_obj(wn.model.get_doclist(dt, dn), 'get_profiles', '', function(r, rt) {
		if(r.exc) {
			msgprint(r.exc);
		} else {
