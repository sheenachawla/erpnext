...
class DocType:
	...
	def create_customer_address(self):
		addr_flds = [self.doc.address_line1, self.doc.address_line2, self.doc.city, 
			self.doc.state, self.doc.country, self.doc.pincode]
	...
	def create_customer_contact(self):
		contact = sql("select distinct name from `tabContact` where customer_name=%s"
		...
		if not contact:
			...
			if self.doc.customer_type == 'Individual':
				self.create_p_contact(self.doc.customer_name,
					self.doc.phone_1,self.doc.email_id,'',self.doc.fax_1,self.doc.address)
			...
			# create primary contact for lead
				...
				self.create_p_contact(
					c_detail and c_detail[0]['lead_name'] or '', 
					c_detail and c_detail[0]['contact_no'] or '', 
					c_detail and c_detail[0]['email_id'] or '', 
					c_detail and c_detail[0]['mobile_no'] or '', 
					c_detail and c_detail[0]['fax'] or '', 
					c_detail and c_detail[0]['address'] or ''
				)
	...
	def delete_customer_address(self):
		for rec in sql("select * from `tabAddress` where customer='%s'" 
				%(self.doc.name), as_dict=1):
			sql("delete from `tabAddress` where name=%s",(rec['name']))
	
	def delete_customer_contact(self):
		for rec in sql("select * from `tabContact` where customer='%s'" 
				%(self.doc.name), as_dict=1):
