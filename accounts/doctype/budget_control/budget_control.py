...
class DocType:
	...
	def check_budget(self,gle,cancel):
		...
		bgt = sql("""select t1.budget_allocated, t1.actual, t2.distribution_id 
		...
		if bgt and bgt[0]['budget_allocated']:
			...
			bgt_flag = sql("""select yearly_bgt_flag, monthly_bgt_flag 
			...
			if bgt_flag and bgt_flag[0]['monthly_bgt_flag'] in ['Stop', 'Warn']:
				...
				start_date = get_value('Fiscal Year', gle['fiscal_year'], \
				 	'year_start_date').strftime('%Y-%m-%d')
				end_date = sql("select LAST_DAY('%s')" % gle['posting_date'])
			
				# get Actual
				actual = get_obj('GL Control').get_period_difference(gle['account'] + '~~~' + \
			...
			sql("""update `tabBudget Detail` set actual = ifnull(actual,0) + %s 
