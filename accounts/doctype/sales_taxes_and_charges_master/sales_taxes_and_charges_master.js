...
cur_frm.pformat.grand_total_print = function(doc) {
	return '';
}

cur_frm.pformat.rounded_total_print = function(doc) {
	return '';
}

cur_frm.pformat.rounded_total_in_words_print = function(doc) {
	return '';
}
...
cur_frm.pformat.taxes_and_charges= function(doc){
  //function to make row of table
	...
	if (!doc.print_without_amount) {
		...
		var cl = getchildren('Sales Taxes and Charges',doc.name,'taxes_and_charges');
		...
		// grand total
		if(!print_hide_dict['grand_total_print']) {
			out += make_row('Grand Total',fmt_money(doc.grand_total_print),1);
		}
		
		if(!print_hide_dict['rounded_total_print']) {
			out += make_row('Rounded Total',fmt_money(doc.rounded_total_print),1);
		}

		if(doc.rounded_total_in_words_print && !print_hide_dict['rounded_total_in_words_print']){
			out +='</table></td></tr>';
			out += '<tr><td colspan = "2">';
			out += '<table><tr><td style="width:25%;"><b>In Words</b></td>'
			out+= '<td style="width:50%;">'+doc.rounded_total_in_words_print+'</td></tr>'
		}
