...
class Cart:
	...
	def guess_territory(self):
		...
		return territory or self.get_settings().doc.territory or "All Territories"
	...
	def make_lead(self):
		lead = webnotes.bean({
			...
			"email_id": webnotes.session.user,
			"lead_name": get_fullname(webnotes.session.user),
			"territory": self.guess_territory(),
			"status": "Open" # TODO: set something better???
		})
	...
	def get_settings(self):
		if not hasattr(self, "_settings"):
			self._settings = webnotes.bean("Shopping Cart Settings", "Shopping Cart Settings")
	...
	def get_quotation(self):
		if not hasattr(self, "_quotation"):
			...
			self._quotation.ignore_permissions = True
				
		# set it in cookies for using in product page
		webnotes.cookies[b"selling_price_list"] = self._quotation.doc.selling_price_list
	...
	def make_quotation(self):
		...
		self.apply_cart_settings(quotation)

		return quotation
		
	def apply_cart_settings(self, quotation=None):
		if not quotation:
			quotation = self.get_quotation()
		
		controller = quotation.make_controller()
		billing_territory = controller.get_billing_territory()
		shipping_territory = controller.get_shipping_territory()
		
		settings = self.get_settings().make_controller()
		
		quotation.doc.selling_price_list = settings.get_price_list(billing_territory)
		quotation.doc.charge = settings.get_tax_master(billing_territory)
		
		shipping_rules = settings.get_shipping_rules(shipping_territory)
		quotation.doc.shipping_rule = shipping_rules and shipping_rules[0] or None

		# clear fields so that they can be fetched again, via onload post render
		for fieldname in ["price_list_currency", "currency", "plc_conversion_rate", "conversion_rate"]:
			quotation.doc.fields[fieldname] = None
		
		for item in quotation.doclist.get({"parentfield": "quotation_details"}):
			for fieldname in ["ref_rate", "adj_rate", "export_rate", "export_amount", "amount"]:
				item.fields[fieldname] = None
		
		quotation.clear_table("other_charges")
		quotation.run_method("onload_post_render")
		quotation.run_method("apply_shipping_rule")
		
		quotation.run_method("calculate_taxes_and_totals")
		
	def add_to_cart(self, item_code):
		self.update_cart(item_code, 1)
		
	def remove_from_cart(self, item_code):
		self.update_cart(item_code, 0)
		
	def update_cart(self, item_code, qty):
		qty = flt(qty)
		quotation = self.get_quotation()
		if qty == 0:
			...
			# if no items left, mark quotation for deletion
			if not (quotation.doclist.get({"parentfield": "quotation_details"}) or \
				quotation.doc.fields.get("__islocal")):
					quotation.doc.fields["__delete"] = True
		else:
			# if existing item
			item = quotation.doclist.get({"item_code": item_code})
			if item:
				item[0].qty = qty
			else:
				quotation.doclist.append({
		...
		if quotation.doc.fields.get("__delete"):
			webnotes.delete_doc("Quotation", quotation.doc.name, ignore_permissions=True)
			del self._quotation
		else:
...
class TestCart(unittest.TestCase):
	def setUp(self):
		...
		# delete existing cart quotation
		cart = Cart()
		quotation = cart.get_quotation()
		if not quotation.doc.fields.get("__islocal"):
			webnotes.delete_doc("Quotation", quotation.doc.name, ignore_permissions=True)
	...
	def test_get_quotation(self):
		...
		# check for price list, taxes and shipping rule
		self.assertEquals(quotation.doc.selling_price_list, "_Test Price List Rest of the World")
		self.assertEquals(quotation.doc.charge, "_Test Sales Taxes and Charges Master")
		self.assertEquals(quotation.doc.shipping_rule, "_Test Shipping Rule - _Test Territory")
		
		# main doc, 2 taxes and 1 shipping charge
		self.assertEquals(len(quotation.doclist), 4)
		
	def test_add_to_cart(self):
		cart = Cart()
		cart.add_to_cart("_Test Item")
		quotation = cart.get_quotation()
		items = quotation.doclist.get({"parentfield": "quotation_details"})
		self.assertEquals(len(items), 1)
		self.assertEquals(items[0].item_code, "_Test Item")
		self.assertEquals(items[0].qty, 1)
		self.assertEquals(items[0].ref_rate, 2)
		self.assertEquals(items[0].adj_rate, 0)
		self.assertEquals(items[0].export_rate, 2)
		self.assertEquals(items[0].export_amount, 2)
	...
	def test_remove_from_cart(self):
		...
		cart.remove_from_cart("_Test Item")
