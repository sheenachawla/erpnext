...
class SupplierController(PartyController):
	...
	def create_supplier_ledger(self):
		for comp in webnotes.conn.sql("select name from `tabCompany`", as_dict=1):
			parent_account = self.get_parent_account(comp['name'])
			self.create_account({'parent_account': parent_account, 'company': comp['name']})

	def get_parent_account(self, company):
		"""
		...
		supp_type_acc = webnotes.conn.get_value('Account', {'account_name': self.doc.supplier_type, \
			'company': company, 'debit_or_credit': 'Credit', 'is_pl_account': 'No'})
		...
		if not supp_type_acc:
			supp_type_acc = self.create_account({
				...
				'parent_account': self.get_party_group(company, 'payables_group'),
				'company': company,
