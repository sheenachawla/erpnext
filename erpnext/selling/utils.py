...
class SalesController(DocListController):
		...
		def _check_conversion_rate(currency, conv_rate, currency_type):
			if not conv_rate or (currency == def_currency and flt(conv_rate) != 1.00) or \
				...
				msgprint("""Please Enter Appropriate Conversion Rate for %s 
					currency (%s) to base currency (%s)""" % (currency_type, self.doc.currency, def_currency), 
					raise_exception = webnotes.ValidationError)
					
		_check_conversion_rate(self.doc.currency, self.doc.conversion_rate, 'customer')
		_check_conversion_rate(self.doc.price_list_currency, self.doc.plc_conversion_rate, 'price list')
	...
	def validate_prevdoc_docstatus(self):
		for d in getlist(self.doclist, 'sales_order_details'):
			cancel_quo = webnotes.conn.sql("""select name from `tabQuotation` 
				where docstatus = 2 and name = '%s'""" % d.prevdoc_docname)
			if cancel_quo:
	...
	def check_if_nextdoc_exists(self, nextdoc_types):
		for d in nextdoc_types:
			...
			if nextdoc:
				msgprint("""Submitted %s: %s exists against this %s. To cancel this document 
					first cancel %s"""
