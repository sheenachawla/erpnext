...
class DocType:
	...
	def validate_asset(self, item_code):
		if webnotes.conn.sql("select is_asset_item from tabItem where name=%s", item_code)[0][0] == 'Yes' and self.doc.warehouse_type != 'Fixed Asset':
	...
	def repost_stock(self):
		bl = webnotes.conn.sql("select name from tabBin where warehouse=%s", self.doc.name)
		for b in bl:
	...
	def validate(self):
		...
		if not self.doc.warehouse_type:
			...
			raise Exception
		wt = webnotes.conn.sql("select warehouse_type from `tabWarehouse` where name ='%s'" % self.doc.name)
		if cstr(self.doc.warehouse_type) != cstr(wt and wt[0][0] or ''):
			webnotes.conn.sql("update `tabStock Ledger Entry` set warehouse_type = '%s' where warehouse = '%s'" % (self.doc.warehouse_type, self.doc.name))
	...
	def on_trash(self):
		...
		if webnotes.conn.sql("select name from `tabStock Ledger Entry` where warehouse = %s and ifnull('is_cancelled', '') = 'No'", self.doc.name):
