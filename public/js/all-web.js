...
wn.request.cleanup=function(opts,r){wn.last_response=r;wn.request.update_btn(opts.btn,null);if(opts.show_spinner)wn.hide_loading();if(opts.freeze)wn.unfreeze();if(wn.boot&&wn.boot.sid&&wn.get_cookie('sid')!=wn.boot.sid){if(!wn.app.logged_out){msgprint('Session Expired. Logging you out');wn.app.logout();}
return;}
...
$(document).ready(function(){wn.versions.check();wn.provide('wn.app');$.extend(wn.app,new wn.Application());});wn.Application=Class.extend({init:function(){var me=this;if(window.app){wn.call({method:'startup',callback:function(r){wn.provide('wn.boot');wn.boot=r.boot;if(wn.boot.profile.name=='Guest'){window.location='index.html';return;}
...
last_date=29;return y+'-'+int_to_str(m,2)+'-'+last_date;},get_user_fmt:function(){var t=sys_defaults.date_format;if(!t)t='dd-mm-yyyy';return t;},get_date_and_time_str:function(val){if(val.search(":")==-1){return[val,''];}else{return val.split(' ');}},str_to_user:function(val,no_time_str){var user_fmt=dateutil.get_user_fmt();if(!val)return val;[datestr,timestr]=dateutil.get_date_and_time_str(val);d=datestr.split('-');if(d.length==3){if(user_fmt=='dd-mm-yyyy')
