...
return val;},set:function(key,val){this.fields[key]=this.convert_type(key,val);$(document).trigger(wn.model.event_name(this.get('doctype'),this.get('name')),[key,this.fields[key]]);}});wn.model.DocList=Class.extend({init:function(doclist){this.doclist=[];if(doclist){for(var i=0,len=doclist.length;i<len;i++){this.add(doclist[i]);}}},setup:function(doc){this.doc=doc;this.doctype=doc.get('doctype');this.name=doc.get('name');wn.provide('wn.doclists.'+this.doctype);wn.doclists[this.doctype][this.name]=this;},add:function(doc){if(!(doc instanceof wn.model.Document)){var doc=new wn.model.Document(doc);}
...
return $.map(this.doclist,function(d){return me.match(filters,d)});},get_value:function(key,def){return this.doc.get(key,def);},match:function(filters,doc){for(key in filters){var fval=filters[key];if(fval instanceof Array){if(!in_list(fval,doc.get(key)))return null;}else{if(doc.get(key)!=filters[key]){return null;}}}
...
me.fieldselect.$select.val(fieldname);var field_area=me.$w.find('.filter_field').empty().get(0);me.field=wn.ui.make_control({docfield:df,parent:field_area});me.field.hide_label();this.set_default_condition(df,fieldtype);$(me.field.wrapper).find(':input').keydown(function(ev){if(ev.which==13){me.flist.listobj.run();}})},set_fieldtype:function(df,fieldtype){if(df.original_type)
...
$(me.txt).autocomplete({source:function(request,response){var tab_name="`tab"+me.df.options+"`"
var filters=[[me.df.options,"name","like",request.term+'%']].concat(me.filters||[]);var search_fields=me.df.search_fields||[];wn.call({method:'webnotes.widgets.doclistview.get',args:{'docstatus':["0","1"],"fields":$.map(["name"].concat(search_fields),function(v){return"`tab"+me.df.options+"`."+strip(v);}),"filters":filters,'doctype':me.df.options},callback:function(r){response($.map(r.message,function(v){return{"label":v.name,"info":$.map(search_fields,function(f){return v[f];}).join(", "),}}));}});},select:function(event,ui){me.set_input_value(ui.item.value);}}).data('autocomplete')._renderItem=function(ul,item){return $('<li></li>').data('item.autocomplete',item).append(repl('<a>%(label)s<br><span style="font-size:10px">%(info)s</span></a>',item)).appendTo(ul);};$(this.txt).change(function(){var val=$(this).val();me.set_input_value_executed=false;console.log('fired:'+me.dialog_fired)
...
this.filters=$.map(wn.model.get(filters),function(d){var val=(d.value.indexOf('field:')==0)&&cur_frm.doc[strip(d.value.slice(6))]||d.value;return[[me.df.options,d.fieldname,d.condition,val]]});}
...
if(me.btn1)me.btn1.onclick=function(){if(me.txt.value&&me.df.options){wn.set_route('Form',me.df.options,me.txt.value);}}
...
LinkField.prototype.set_input_value=function(val){var me=this;me.set_input_value_executed=true;me.refresh_label_icon();if(me.not_in_form){$(this.txt).val(val);return;}
...
LinkField.prototype.set_disp=function(val){var t=null;if(val)t="<a href='#Form/"+this.df.options+"/"+val+"'>"+val+"</a>";this.set_disp_html(t);}
...
me.btn.setAttribute('title','Search');if(iconsrc1){var c2=tab.rows[0].insertCell(2);$y(c2,{width:'20px'});me.btn1=$a(c2,'i',iconsrc1,icon_style)
...
wn.ui.Control=Class.extend({init:function(opts){$.extend(this,opts);this.make();this.set_events();this.apply_hidden();this.apply_mandatory();this.set_init_value();this.set_change_event();this.toggle_editable(false);},make:function(){if(this.docfield.vertical){this.make_body_vertical();}else{this.make_body();}
this.make_input();this.make_label();},make_label:function(){this.$w.find('label').text(this.docfield.label);if(this.no_label){this.hide_label();}else{if(this.docfield.description){this.help_block(this.docfield.description);}}},set_change_event:function(){var me=this;if(this.$input)
 ...
 */
wn.ui.IntControl=wn.ui.Control.extend({validate:function(val){return cint(val);}});wn.ui.FloatControl=wn.ui.Control.extend({validate:function(val){if(val===null)val=0;return parseFloat(val).toFixed(6);}});wn.ui.CurrencyControl=wn.ui.Control.extend({validate:function(val){if(val===null)val=0;return parseFloat(val).toFixed(6);}});wn.ui.CheckControl=wn.ui.Control.extend({make_input:function(){this.$input=$('<input type="checkbox">').appendTo(this.$w.find('.controls'));}});wn.ui.TextControl=wn.ui.Control.extend({make_input:function(){this.$input=$('<textarea type="text" rows="5">').appendTo(this.$w.find('.controls'));}});wn.ui.SelectControl=wn.ui.Control.extend({make_input:function(){this.$input=$('<select>').appendTo(this.$w.find('.controls'));this.$input.add_options(this.docfield.options.split('\n'));}});wn.ui.ButtonControl=wn.ui.Control.extend({make_input:function(){this.hide_label();this.$input=$('<button class="btn btn-small">').html(this.docfield.label).appendTo(this.$w.find('.controls'));},get_value:function(){return null;},toggle_editable:function(){},set_static:function(){},});
/*
 *	lib/js/wn/form/control_link.js
 */
wn.ui.LinkControl=wn.ui.Control.extend({make_input:function(){var me=this;this.$input_wrap=$('<div class="input-append">').appendTo(this.$w.find('.controls'));this.$input=$('<input type="text" />').appendTo(this.$input_wrap);this.$button=$('<button class="btn"><i class="icon-search"></i></button>').appendTo(this.$input_wrap).click(function(){me.search_dialog=new wn.ui.Search({doctype:me.docfield.options,txt:me.$input.val(),with_filters:me.filters,df:me.docfield,callback:function(val){me.set(val);}});return false;});},toggle_input:function(show){this.$input_wrap.toggle(show);}});
/*
 *	lib/js/wn/form/control_richtext.js
 */
wn.ui.RichTextControl=wn.ui.Control.extend({init:function(opts){opts.docfield.vertical=true;this._super(opts);},make_input:function(){var me=this;this.$input_wrap=$('<div>').appendTo(this.$w.find('.controls'));this.$input=$('<textarea type="text">').css('font-size','12px').appendTo(this.$input_wrap);this.myid=wn.dom.set_unique_id(this.$input.get(0));this.$input.tinymce({script_url:'js/lib/tiny_mce_33/tiny_mce.js',theme:"advanced",plugins:"style,inlinepopups,table,advimage",extended_valid_elements:"div[id|dir|class|align|style]",width:'100%',height:'360px',theme_advanced_buttons1:"bold,italic,underline,strikethrough,hr,|,justifyleft,justifycenter,justifyright,|,formatselect,fontselect,fontsizeselect,|,image",theme_advanced_buttons2:"bullist,numlist,|,outdent,indent,|,undo,redo,|,link,unlink,code,|,forecolor,backcolor,|,tablecontrols",theme_advanced_buttons3:"",theme_advanced_toolbar_location:"top",theme_advanced_toolbar_align:"left",content_css:"js/lib/tiny_mce_33/custom_content.css",oninit:function(){me.init_editor();}});},get:function(){if(this.editor)return this.editor.getContent();else return this.$input.val();},set_input:function(v){if(this.editor)this.editor.setContent(v);else this.$input.val(v);},init_editor:function(){var me=this;this.editor=tinymce.get(this.myid);this.editor.onKeyUp.add(function(ed,e){me.set(ed.getContent());});this.editor.onPaste.add(function(ed,e){me.set(ed.getContent());});this.editor.onSetContent.add(function(ed,e){me.set(ed.getContent());});if(this.doc){this.editor.setContent(this.doc.get(this.docfield.fieldname));}},toggle_input:function(show){this.$input_wrap.toggle(show);}});
/*
 *	lib/js/wn/form/control_grid.js
 */
wn.ui.GridControl=wn.ui.Control.extend({init:function(opts){opts.docfield.vertical=true;this.tabletype=opts.docfield.options;this._super(opts);},make_input:function(){wn.lib.import_slickgrid();var width=$(this.parent).parent('form:first').width();this.$w=$('<div style="height: 300px; border: 1px solid grey;"></div>').appendTo(this.$w.find('.controls')).css('width',width);var options={enableCellNavigation:true,enableColumnReorder:false,enableRowReordering:true,rowHeight:32,editable:false};this.grid=new Slick.Grid(this.$w.get(0),[],this.get_columns(),options);this.setup_drag_and_drop();},get_columns:function(){var columns=$.map(wn.model.get('DocType',this.tabletype).get({doctype:'DocField'}),function(d){if(!d.hidden){return{id:d.get('fieldname'),field:d.get('fieldname'),name:d.get('label'),width:100}}else{return null;}});function EditButtonFormatter(row,cell,value,columnDef,data){return repl('<button class="btn btn-small grid-edit" \
    data-parentfield="%(parentfield)s" data-name="%(name)s">Edit</button>',data);}
return[{id:"#",name:"",width:40,behavior:"selectAndMove",selectable:false,resizable:false,cssClass:"cell-reorder dnd"},{id:'_edit',field:'_edit',name:'',width:55,formatter:EditButtonFormatter},{id:'idx',field:'idx',name:'Sr',width:40}].concat(columns);},set_init_value:function(){this.set();},get_data:function(){var data=wn.model.get(this.doc.get('doctype'),this.doc.get('name')).get({parentfield:this.docfield.fieldname});data=$.map(data,function(d){return d.fields;});data.sort(function(a,b){return a.idx>b.idx;});return data;},set:function(){var me=this;this.grid.setData(this.get_data());this.grid.render();this.$w.find('.grid-edit').on('click',function(){var d=wn.model.get(me.doc.get('doctype'),me.doc.get('name')).get({parentfield:$(this).attr('data-parentfield'),name:$(this).attr('data-name'),})[0];var form_dialog=new wn.views.FormDialog({title:'Editing row #'+d.get('idx'),doc:d});form_dialog.show();return false;})},setup_drag_and_drop:function(){var grid=this.grid;var me=this;grid.setSelectionModel(new Slick.RowSelectionModel());var moveRowsPlugin=new Slick.RowMoveManager();moveRowsPlugin.onBeforeMoveRows.subscribe(function(e,data){for(var i=0;i<data.rows.length;i++){if(data.rows[i]==data.insertBefore||data.rows[i]==data.insertBefore-1){e.stopPropagation();return false;}}
...
wn.views.FormDialog=wn.ui.Dialog.extend({init:function(opts){$.extend(this,opts);wn.get_or_set(this,'width',600);wn.get_or_set(this,'title',this.name);this._super();this.form=new wn.ui.Form({doc:this.doc,fields:this.fields,parent:this.body,appframe:this.appframe,dialog:this});}})
wn.views.FormPage=Class.extend({init:function(doctype,name){this.make_page();this.set_breadcrumbs(doctype,name);this.form=new wn.ui.Form({doc:wn.model.get(doctype,name).doc,parent:this.$w,appframe:this.page.appframe});},set_breadcrumbs:function(doctype,name){wn.views.breadcrumbs(this.page.appframe,wn.model.get_value('DocType',doctype,'module'),doctype,name);},make_page:function(){var page_name=wn.get_route_str();this.page=wn.container.add_page(page_name);wn.ui.make_app_page({parent:this.page});wn.container.change_to(page_name);this.$w=$(this.page).find('.layout-main-section');}});wn.ui.Form=Class.extend({init:function(opts){$.extend(this,opts);this.controls={};if(this.doc){this.fields=$.map(wn.model.get('DocType',this.doc.get('doctype')).get('DocField',{}),function(d){return d.fields;});}
this.make_form();this.make_toolbar();this.listen();},make_toolbar:function(){var me=this;this.appframe.add_button('Save',function(){if(me.doc.get('parent')){wn.forms[me.doc.get('parenttype')][me.doc.get('parent')].form.controls[me.doc.get('parentfield')].set();if(me.dialog.hide)me.dialog.hide();}else{var btn=this;$(this).html('Saving...').attr('disabled','disabled');wn.model.get(me.doc.get('doctype'),me.doc.get('name')).save(0,function(){$(this).attr('disabled',false).html('Save');});}});},make_form:function(){var me=this;this.$form=$('<form class="form-horizontal">').appendTo(this.parent);if(this.fields[0].fieldtype!='Section Break'){me.make_fieldset('_first_section');}
...
this.last_fieldset=$fset;},listen:function(){var me=this;if(this.doc){$(document).bind(wn.model.event_name(this.doc.get('doctype'),this.doc.get('name')),function(ev,key,val){if(me.controls[key])me.controls[key].set_input(val);});}},save:function(callback){}});
/*
...
me.rename_notify(dt,old_name,new_name)});Mousetrap.bind(['ctrl+s','command+s'],function(e){if(cur_frm){cur_frm.save();return false;}});Mousetrap.bind(['ctrl+n','command+n'],function(){if(cur_frm){new_doc(cur_frm.doctype);return false;}});Mousetrap.bind(['ctrl+l','command+l'],function(){if(cur_frm){wn.set_route('List',cur_frm.doctype);return false;}});}
...
_f.Frm.prototype.set_value_in_locals=function(dt,dn,fn,v){var d=locals[dt][dn];if(!d){console.log("No "+dt+": "+dn);return;}
var changed=d[fn]!=v;if(changed&&((d[fn]==null&&v=='')||(d[fn]==''&&v==null)))
...
_f.Frm.prototype.set_child_df_property=function(parent_type,fieldname,property,value){var field=wn.meta.get_docfield(parent_type,fieldname)
field[property]=value;}
_f.Frm.prototype.toggle_enable=function(fnames,enable){cur_frm.field_map(fnames,function(field){field.disabled=enable?0:1;});}
_f.Frm.prototype.toggle_reqd=function(fnames,mandatory){cur_frm.field_map(fnames,function(field){field.reqd=mandatory?1:0;});}
...
this.add_attachment=function(){if(!this.dialog){this.dialog=new wn.ui.Dialog({title:'Add Attachment',width:400})
