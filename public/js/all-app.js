...
var label=docfield?docfield.get('label'):field;if(label=='_user_tags')label='Tags';var $w=$('<div class="stat-wrapper">\
   <h4>'+label+'</h4>\
   <div class="stat-grid">\
   </div>\
 ...
 */
wn.ui.GridControl=wn.ui.Control.extend({init:function(opts){opts.docfield.vertical=true;this.tabletype=opts.docfield.options;this._super(opts);},make_input:function(){wn.lib.import_slickgrid();var width=$(this.parent).parent('form:first').width();this.$w=$('<div style="height: 300px; border: 1px solid grey;"></div>').appendTo(this.$w.find('.controls')).css('width',width);var options={enableCellNavigation:true,enableColumnReorder:false,rowHeight:32,editable:false};this.grid=new Slick.Grid(this.$w.get(0),[],this.get_columns(),options);},get_columns:function(){var columns=$.map(wn.model.get('DocType',this.tabletype).get({doctype:'DocField'}),function(d){if(!d.hidden){return{id:d.get('fieldname'),field:d.get('fieldname'),name:d.get('label'),width:100}}else{return null;}});function EditButtonFormatter(row,cell,value,columnDef,data){return repl('<button class="btn btn-small grid-edit" \
    data-parentfield="%(parentfield)s" data-name="%(name)s">Edit</button>',data);}
return[{id:'_edit',field:'_edit',name:'',width:55,formatter:EditButtonFormatter},{id:'idx',field:'idx',name:'Sr',width:40}].concat(columns);},set_init_value:function(){this.set();},set:function(){var me=this;var rows=wn.model.get(this.doc.get('doctype'),this.doc.get('name')).get({parentfield:this.docfield.fieldname});this.grid.setData($.map(rows,function(d){return d.fields;}));this.grid.render();this.$w.find('.grid-edit').on('click',function(){var d=wn.model.get(me.doc.get('doctype'),me.doc.get('name')).get({parentfield:$(this).attr('data-parentfield'),name:$(this).attr('data-name'),})[0];var form_dialog=new wn.views.FormDialog({title:'Editing row #'+d.get('idx'),doc:d});form_dialog.show();return false;})}})
/*
...
if(wn.model.get('DocType',dt).get_value('in_dialog')){var form_dialog=new wn.views.FormDialog({doc:wn.model.get(dt,dn).doc});form_dialog.show();}else{var page_name=wn.get_route_str();if(wn.contents[page_name]){wn.container.change_to(page_name);}else{wn.get_or_set(wn.forms,dt,{})[dn]=new wn.views.FormPage(dt,dn);}}});})},create:function(dt){var new_name=LocalDB.create(dt);wn.set_route('Form',dt,new_name);}}
